var Zepto = function () {
    ...
    var a, b, c, d, A, B, e = [], f = e.slice, g = e.filter, h = window.document, i = {}, j = {},
        k = {"column-count": 1, columns: 1, "font-weight": 1, "line-height": 1, opacity: 1, "z-index": 1, zoom: 1},
        l = /^\s*<(\w+|!)[^>]*>/, m = /^<(\w+)\s*\/?>(?:<\/\1>|)$/,
        n = /<(?!area|br|col|embed|hr|img|input|link|meta|param)(([\w:]+)[^>]*)\/>/gi, o = /^(?:body|html)$/i,
        ...
        v = /complete|loaded|interactive/, w = /^[\w-]*$/, x = {}, y = x.toString, z = {}, C = h.createElement("div"),
        D = {
            tabindex: "tabIndex",
            readonly: "readOnly",
            "for": "htmlFor",
...
window.Zepto = Zepto, void 0 === window.$ && (window.$ = Zepto), function (a) {
    ...
    var d, e, y, b = 0, c = window.document, f = /<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi,
    ...
    a.active = 0, a.ajaxJSONP = function (d, e) {
        ...
        var j, m, f = d.jsonpCallback, g = (a.isFunction(f) ? f() : f) || "jsonp" + ++b, h = c.createElement("script"),
    ...
    function b(a) {
        var b = this.os = {}, c = this.browser = {}, d = a.match(/Web[kK]it[\/]{0,1}([\d.]+)/),
            e = a.match(/(Android);?[\s\/]+([\d.]+)?/), f = !!a.match(/\(Macintosh\; Intel /),
            g = a.match(/(iPad).*OS\s([\d_]+)/), h = a.match(/(iPod)(.*OS\s([\d_]+))?/),
            i = !g && a.match(/(iPhone\sOS)\s([\d_]+)/), j = a.match(/(webOS|hpwOS)[\s\/]([\d.]+)/),
            k = a.match(/Windows Phone ([\d.]+)/), l = j && a.match(/TouchPad/), m = a.match(/Kindle\/([\d.]+)/),
            n = a.match(/Silk\/([\d._]+)/), o = a.match(/(BlackBerry).*Version\/([\d.]+)/),
            p = a.match(/(BB10).*Version\/([\d.]+)/), q = a.match(/(RIM\sTablet\sOS)\s([\d.]+)/),
            r = a.match(/PlayBook/), s = a.match(/Chrome\/([\d.]+)/) || a.match(/CriOS\/([\d.]+)/),
            t = a.match(/Firefox\/([\d.]+)/),
    ...
    var d, k, l, m, n, o, p, q, r, s, c = "", g = {Webkit: "webkit", Moz: "", O: "o"}, h = window.document,
...
document.addEventListener("touchmove", function (a) {
    ...
    function g() {
        var b = parseFloat($(this).attr("changeX")), c = parseFloat($(this).attr("changeY")),
            e = parseFloat($(this).attr("rotateX")), f = parseFloat($(this).attr("rotateY"));
